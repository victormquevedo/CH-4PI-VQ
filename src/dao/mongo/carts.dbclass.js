...
class CartsManager {
  ...
  static #isProductInCart = (cartToUpdate, pid) =>
    cartToUpdate.products.some((product) => product.id === pid);
  static #findProductIndexInCart = (cartToUpdate, pid) =>
    cartToUpdate.products.findIndex((product) => product.id === pid);
    ...
    try {
      ...
      for (const product of products) {
        ...
        if ([pid, quantity].includes(undefined)) {
          throw new Error(
            `${products.indexOf(product) + 1}° product with incomplete mandatory fields`
          );
        }
        if (CartsManager.#isProductInCart(cartToUpdate, pid)) {
  ...
  updateProductQuantityFromCart = async (id, pid, { quantity }) => {
    try {
      ...
      cartToUpdateProductQuantity.products[
        CartsManager.#findProductIndexInCart(cartToUpdateProductQuantity, pid)
      ].quantity = quantity;
      await cartsModel.updateOne({ id }, { $set: cartToUpdateProductQuantity });
      return { message: 'Product quantity updated' };
    } catch (err) {
      throw new CustomError(
        errorsDict.DATABASE_ERROR,
        `updateProductQuantityFromCart - ${err}`,
        true
      );
    }
  };
